...
savePath = 'VolumiDCE_Sub_AllBox_proporzione';
name = t.name;
...
for i=1:numel(name)
    ...
    %il caso base è costituito da 4 acquisizioni post contrasto
    %sottrattive, quindi 4:numeroAcquisizioni = indice:x, dove x indica
    %l'acquisizione da considerare e indice va da 1 a 4. Quindi
    %x=(numeroAcquisizioni*indice)/4
    acquisizioni = [round((numeroAcquisizioni*1)/4) round((numeroAcquisizioni*2)/4) round((numeroAcquisizioni*3)/4) round((numeroAcquisizioni*4)/4)];
    %----------------------------------------> selezione delle acquisizioni
    %acquisizioni = 1:numeroAcquisizioni;
    disp(['        Acqusition Indeces   ' num2str(acquisizioni) ]);
    selectedAcquisition = acquisizioni;
    ...
    if sum(mask_basso(:)) > 0
        imdb = extractEachLesion(MRIvolume, mask_basso, specificPath, row.name{1}, 'b', x_size, z_size, dcemriPath, selectedAcquisition);
    end
    
    if sum(mask_alto(:)) > 0
        imdb = extractEachLesion(MRIvolume, mask_alto, specificPath, row.name{1}, 'a', x_size, z_size, dcemriPath, selectedAcquisition);
    end
...
s= split(patientName,'.');
pathBase = ['Immagini_AllBox_proporzione' filesep s{1} '_' s{2} '_' sub];
